...
class Network(object):
    ...
    def __init__(self, num_features, num_class, fc_hidden_units, apply_dropout, ewc_batch_size=100, ewc_batches=550):
        ...
        self.saver = tf.compat.v1.train.Saver(max_to_keep=1000, var_list=self.theta + self.theta_lagged + self.fisher_diagonal)
    ...
    def fc_feedforward(self, h, biases, weights, apply_dropout):
        if apply_dropout:
            h = tf.nn.dropout(h, 1 - (self.keep_prob_input))
        for (w, b) in list(zip(weights, biases))[:-1]:
            h = self.create_fc_layer(h, w, b)
            if apply_dropout:
                h = tf.nn.dropout(h, 1 - (self.keep_prob_hidden))
    ...
    def unaggregated_nll(self):
        ...
        for (x, y, biases, weights) in zip(x_examples, y_examples, biases_per_example, weights_per_example):
            ...
            nll = - tf.reduce_sum(input_tensor=y * tf.nn.log_softmax(scores))
            nll_list.append(nll)
        nlls = tf.stack(nll_list)
        return tf.reduce_sum(input_tensor=nlls), biases_per_example, weights_per_example

    def fisher_minibatch_sum(self, nll_per_example, biases_per_example, weights_per_example):
        bias_grads_per_example = [tf.gradients(ys=nll_per_example, xs=biases) for biases in biases_per_example]
        weight_grads_per_example = [tf.gradients(ys=nll_per_example, xs=weights) for weights in weights_per_example]
    ...
    def create_fisher_ops(self):
        ...
        self.fisher_accumulate_op = [tf.compat.v1.assign_add(f1, f2) for f1, f2 in zip(self.fisher_diagonal, self.fisher_minibatch)]
        scale = 1 / float(self.ewc_batches * self.ewc_batch_size)
        self.fisher_full_batch_average_op = [tf.compat.v1.assign(var, scale * var) for var in self.fisher_diagonal]
        self.fisher_zero_op = [tf.compat.v1.assign(tensor, tf.zeros_like(tensor)) for tensor in self.fisher_diagonal]

    @staticmethod
    def create_fc_layer(input, w, b, apply_relu=True):
        with tf.compat.v1.name_scope('fc_layer'):
    ...
    def create_variable(shape, name, c=None, sigma=None, trainable=True):
        if sigma:
            initial = tf.random.truncated_normal(shape, stddev=sigma, name=name)
        else:
    ...
    def create_fc_variables(self):
        with tf.compat.v1.name_scope('fc_variables'):
        ...
        with tf.compat.v1.name_scope('fc_variables_lagged'):
    ...
    def create_placeholders(self):
        with tf.compat.v1.name_scope("prediction-inputs"):
            self.x = tf.compat.v1.placeholder(tf.float32, [None, self.num_features], name='x-input')
            self.y = tf.compat.v1.placeholder(tf.float32, [None, self.num_class], name='y-input')
        with tf.compat.v1.name_scope("dropout-probabilities"):
            self.keep_prob_input = tf.compat.v1.placeholder(tf.float32)
            self.keep_prob_hidden = tf.compat.v1.placeholder(tf.float32)
        with tf.compat.v1.name_scope("fisher-inputs"):
            self.x_fisher = tf.compat.v1.placeholder(tf.float32, [self.ewc_batch_size, self.num_features])
            self.y_fisher = tf.compat.v1.placeholder(tf.float32, [self.ewc_batch_size, self.num_class])